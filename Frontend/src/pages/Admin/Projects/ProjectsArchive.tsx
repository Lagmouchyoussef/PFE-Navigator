    ...
    <div className="projects-modern-layout py-4">
      <Container fluid className="px-4">
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3">
          <div>
            <h2 className="fw-bold mb-1 text-gradient">Archives des Projets</h2>
          ...
          {filteredProjects.map((project, i) => (
            <Col key={project.id} lg={4} md={6}>
              <motion.div
                ...
                className="h-100"
              >
                <div className="glass-card rounded-4 overflow-hidden h-100 shadow-sm d-flex flex-column transition-all hover-translate">
                  <div className="p-4 flex-grow-1">
                    <div className="d-flex justify-content-between align-items-start mb-3">
                      <div className="p-2 bg-primary bg-opacity-10 text-primary rounded-3">
                        <Briefcase size={20} />
                      </div>
                      <Badge bg={project.status === 'Completed' ? 'success' : 'danger'} className={`bg-opacity-10 text-${project.status === 'Completed' ? 'success' : 'danger'} border border-${project.status === 'Completed' ? 'success' : 'danger'} border-opacity-25 extra-small rounded-pill`}>
                    ...
                    <p className="text-muted extra-small mb-4 line-clamp-2 fw-bold opacity-75" style={{ minHeight: '40px' }}>
                      {project.desc}
                    </p>
